...
bot = telebot.TeleBot('6231506245:AAEzy5ALjYCwTZR7ozIgSLrZh4CNbnTlw6c')
...
def get_text_message(message):
    if message.text == '👋 Поздороваться':
        ...
        btn1 = types.KeyboardButton('Как стать автором на Хабр?')
        btn2 = types.KeyboardButton('Правила сайта')
        btn3 = types.KeyboardButton('Советы по оформлению сайта')
        markup.add(btn1, btn2, btn3)
        bot.send_message(message.from_user.id, '❓ Задайте интересующий вопрос', reply_markup=markup) #otvet bota

    elif message.text == 'Как стать автором на Хабр?':
    ...
    elif message.text == 'Советы по оформлению сайта':
